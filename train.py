...
from MVPNet.model import MVPNet
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='MVPNet')#
        ...
        # save model for <testing>
            if mIoU > best_miou:
                ...
                if args.shot == 1:
                    ...
                    filename = args.snapshot_path + '/train{}_epoch_'.format(args.eval_shot) + str(epoch) + '_{:.4f}'.format(best_miou) + '.pth'
                if main_process():                    
                    logger.info('Saving checkpoint to: ' + filename)
                    torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, filename)   
