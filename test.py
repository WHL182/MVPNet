...
from MVPNet.model import MVPNet_eval

from MVPNet.util import dataset
from MVPNet.util import transform, transform_tri, config
from MVPNet.util.util import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, get_model_para_number, setup_seed, get_logger, get_save_path, \
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='MVPNet_eval')
...
# ----------------------  DATASET  ----------------------
    ...
    if args.evaluate:
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(split=args.split, shot=args.eval_shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.val_list, \
                                    transform=val_transform, transform_tri=val_transform_tri, mode='val', ann_type=args.ann_type, data_set=args.data_set, use_split_coco=args.use_split_coco)
