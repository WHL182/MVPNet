...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0_resnet50.yaml', help='config file') # coco/coco_split0_resnet101.yaml
...
def validate_GFSS(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target_t, s_input, s_mask, subcls, ori_label_t) in enumerate(val_loader):
            ...
            pre = merge_out.cpu().numpy()
            tag = target_t.cpu().numpy()
            print(list(set(list(pre.flatten()))))
            print(list(set(list(tag.flatten()))))
            exit()
